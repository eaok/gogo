...
func main() {
	...
	txn.If(clientv3.Compare(clientv3.CreateRevision("/lock/resource1"), "=", 0)).
		Then(clientv3.OpPut("/lock/resource1", "true", clientv3.WithLease(leaseId))).
		Else(clientv3.OpGet("/lock/resource1")) //否则抢锁失败
